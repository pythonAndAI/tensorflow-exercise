...
#处理好之后的数据文件
INPUT_DATA = "E:\\Alls\\software\\flower_photo\\flower_processed_data.npy"
#保存训练好的模型的路径，这里可以将使用新数据训练得到的完整模型保存下来，如果计算资源充足，还可以在训练完最后的全连接层之后再训练所有网络层，这样可以使得新模型更加贴近新数据
TRAIN_FILE = "E:\\Alls\\software\\model\\save"
#谷歌提供的训练好的模型文件地址
CKPT_FILE = "E:\\Alls\\software\\model\\inception\\inception_v3.ckpt"
...
def main(argv=None):
    ...
    #定义加载模型的函数
    load_fn = slim.assign_from_checkpoint_fn(CKPT_FILE, get_tune_variables(), ignore_missing_vars=True)
    #定义保存新的训练好的模型函数
